    ...
    for sku in end_dates[date]:
        ...
        for store in current_stock:
            if sku in current_stock[store]:
                ...
                if date_datetime > pd.to_datetime("2019-03-20", format='%Y-%m-%d') and sku == "100552534000002":
                    print(f"store {store} sku {sku} loose {loose[sku]} and current_stock {current_stock[store][sku]}")
...
def kill_and_save_results(accumulated_stocks: dict, d_wo_inv: dict, d_wo_inv_wo_wh: dict, Ex_i_s_r: dict,
                          ...
                          MissedSales: dict,
                          base_path: str ,clearml_task,
                          simulation_name: str = 'run1', lamda: float = 0.1,
                          ) -> tuple[
    ...
    if clearml_task is None:
        print("saving artifact of date {} to local".format(date_))
        file_path = os.path.join(simulation_dir, f'{date_}.pkl')
        with open(file_path, 'wb') as f:
            pickle.dump(final_kpi_res, f)
    else:
        print("uploading artifact of date {} to clearml".format(date_))
        clearml_task.upload_artifact(f'{date_}.pkl', artifact_object=final_kpi_res)
    return accumulated_stocks, d_wo_inv, d_wo_inv_wo_wh, Ex_i_s_r, avg_integral_diff, Ex_total_days_wo_inv, loose, MissedSales


# %%
def apply_strategy_naive_bayes(dict_stocks: dict, AshlonStock: dict, ActiveStores: dict,
    ...
    for store in current_stores_replenished:
        ...
        for sku in dict_stocks[store].keys():
            ...
            dict_stocks, AshlonStock, accumulated_stocks = update_AshlonStock_waerhouse(
                potential_stock_order_from_warehouse, dict_stocks, AshlonStock, accumulated_stocks, sku, store, date)
    return dict_stocks, AshlonStock, accumulated_stocks


# %%
def main_simulation_benchmark(dict_deliveries_from_warehouse: dict, dict_arrivals_store_deliveries: dict, skus_simulation: list,
                    dict_sales: dict, dict_stocks: dict, start_dates: dict, end_dates: dict,base_path: str, clearml_task=None) -> None:
    """
    ...
    for date in pd.date_range(min_start_date_copy, end_date, freq="D"):
        ...
        accumulated_stocks, d_wo_inv, d_wo_inv_wo_wh, Ex_i_s_r, avg_integral_diff, Ex_total_days_wo_inv, loose, MissedSales = kill_and_save_results(
            ...
            date_str, end_dates, MissedSales, base_path, clearml_task)
